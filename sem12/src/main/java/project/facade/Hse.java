...
import project.factories.ReportExporterFactory;
import project.factories.TransportExporterFactory;
import project.factories.cars.*;
import project.factories.catamarans.*;
...
import project.services.cars.HseCarService;
import project.services.catamarans.HseCatamaranService;
import project.services.CustomerService;
import project.observers.SalesObserver;
...
/**
 * Фасад для работы с системой продажи транспортных средств. Предоставляет
 * упрощенный интерфейс для управления клиентами, транспортом и процессами
 * продаж.
 */
@Component
...
public class Hse {
    ...
    public void addCustomer(String name, int legPower, int handPower, int iq) {
        Customer customer = Customer.builder().name(name).legPower(legPower)
                .handPower(handPower).iq(iq).build();
    ...
    public Car addPedalCar(int pedalSize) {
        return carService.addCar(pedalCarFactory,
                new PedalEngineParams(pedalSize));
    }
    ...
    public Car addLevitationCar() {
        return carService.addCar(levitationCarFactory,
    ...
    private Catamaran createCatamaran() {
        ...
        case 0 -> catamaranService.addCatamaran(handCatamaranFactory,
                EmptyEngineParams.DEFAULT);
        case 1 -> catamaranService.addCatamaran(pedalCatamaranFactory,
                new PedalEngineParams(6));
        case 2 -> catamaranService.addCatamaran(levitationCatamaranFactory,
                EmptyEngineParams.DEFAULT);
    ...
    public Catamaran addPedalCatamaran(int pedalSize) {
        return catamaranService.addCatamaran(pedalCatamaranFactory,
                new PedalEngineParams(pedalSize));
    }
    ...
    public Catamaran addLevitationCatamaran() {
        return catamaranService.addCatamaran(levitationCatamaranFactory,
                EmptyEngineParams.DEFAULT);
    }

    /**
     * Запускает процесс продажи доступного транспорта. Автомобили продаются
     * перед катамаранами.
     */
    public void sell() {
        carService.sellCars();
